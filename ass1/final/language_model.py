...
if __name__ == '__main__':
    ...
    # parser.add_argument('--smooth',type=int,required=True)
    ...
    n_model = n_gram_model.n_gram_main()
    lm = Language_Model(n_model, n_gram)
    print("Perplexity without tokens: ",end="")
    print(get_perplexity(n_gram,sent.strip(),smoothing))

    sent="<SOS> "
    xxx=""
    for i in range(n_gram-1): 
                xxx+=sent
    sent=xxx+sent.strip()+" <EOS>"
    print("Perplexity with tokens: ",end="")
    print(get_perplexity(n_gram,sent.strip(),smoothing))
