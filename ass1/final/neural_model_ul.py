...
Original file is located at
  https://colab.research.google.com/drive/1BFinKSK2AARpcKns4n9bNY_U7ryOSrnn?usp=sharing
"""
import os
import sys
...
# Global declarations
filename="ulys_neural.txt"
import regex as re
from keras.layers import Embedding, LSTM, Dense
from keras.models import Sequential
import numpy as np
model_path=sys.argv[1]
# need tokenised filename too for tokens, the tokenised files are saved 
# filename=sys.argv[2]
# if(re.search("ulys",filename)==True)
f = open(filename, "r")
data = f.read()

arr = re.findall("<SOS>(.*?)<EOS>", data)
maxlen = min(500,max([len(seq) for seq in arr]))
np.random.seed(41)
train_split=[]
test_split=[]
dev_split=[]
...
l={}
len_d = len(vocab_dict)
# for i in range(0,int(len_d/2)):
#   l[vocab_dict[i][0]]=l[vocab_dict[i][1]]
# if '<UNK>' not in l:
#    l['<UNK>']=0
# for i in range(len(vocab_dict)/2,len(vocab_dict)):
#   l['<UNK>']+=l[vocab_dict[i][1]]
# vocab_dict=l.copy()
# print(vocab_dict)
...
# model.save('ul_final_model_name.h5')
...
model = load_model('final/final_model_name.h5')
...
print(get_perplexity("i am a women"))
# avg=0
# smoothing="neural_model"
# out_path =  "2020115005"  +"_"+"test"  + "_" + smoothing +"_"+ filename
# f = open(out_path, "w")
# f=open(out_path,"a")
...
# avg=0
# smoothing="neural_model"
# out_path =  "2020115005"  +"_"+"train"  + "_" + smoothing +"_"+ filename
# f = open(out_path, "w")
# f=open(out_path,"a")
