...
def KneyserNey(n, ngram):
    ...
    val=-1
    if(n == 1):
        val= kn_base[tuple(ngram)]/kn_base_sum
    elif(n >= 2):
        val =first_term + (lamb_term * KneyserNey(n-1, ' '.join(ngram[1:])))
    print(val)
    return val
...
# write_output("../data/p.txt", "./outputs/euro_kn_test.txt", "k")
