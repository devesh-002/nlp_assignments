...
def train_cbow_word2vec(vocab_size, data, hidden_size=64, batch_size=32,lr=0.001, num_epochs=10, window_size=2, embedding_size=64):
    w2v = cbow_word2vec(vocab_size, embedding_size, hidden_size, PAD_VALUE).to(device)
...
def train_skipgram_word2vec(vocab_size, data, hidden_size=64, batch_size=32,lr=0.001, num_epochs=10, window_size=2, embedding_size=64):
    w2v = skipgram_word2vec(vocab_size, embedding_size, hidden_size, PAD_VALUE).to(device)
    dataload = DataLoader(data, batch_size=batch_size, num_workers=0)
    loss_func = nn.NLLLoss().to(device)
    optim = torch.optim.Adam(params=w2v.parameters(), lr=lr, weight_decay=1e-4)
    for epoch in tqdm(range(num_epochs), desc='epoch'):
...
def plot_tsne(word_embeddings, word_to_idx, words):
    word_vectors = []
    for word in words:
        try:
            idx = word_to_idx[word]
...
if __name__ == '__main__':   
    ...
    # CBOW
    w2v_cbow = train_cbow_word2vec(datasets.vocab_size, datasets.data)
    
    # SKIPGRAM
    w2v_skipgram = train_cbow_word2vec(datasets.vocab_size, datasets.data_skip)
